...
// added by Beren
#include "crazyswarm/MotorControl.h"
...
public:
    ...
    , m_initializedPosition(false)
  {
    ...
    m_subscribeCmdPwm = n.subscribe(tf_prefix + "/cmd_pwm", 1, &CrazyflieROS::cmdPwmSetpoint, this);
...
public:
  ...
  // added by Beren
  void cmdPwmSetpoint(
    const crazyswarm::MotorControl::ConstPtr& msg)
  {
	  uint16_t pwm1 = std::min<uint16_t>(std::max<float>(msg->pwm1, 0.f), 65535);
	  uint16_t pwm2 = std::min<uint16_t>(std::max<float>(msg->pwm2, 0.f), 65535);
	  uint16_t pwm3 = std::min<uint16_t>(std::max<float>(msg->pwm3, 0.f), 65535);
	  uint16_t pwm4 = std::min<uint16_t>(std::max<float>(msg->pwm4, 0.f), 65535);

	  //uint32_t packed = ((m1 & 0xFF00) >> 8) + ((m2 & 0xFF00)) + ((m3 & 0xFF00) << 8) + ((m4 & 0xFF00) << 16);

	  ROS_INFO("pwm command sent!");

	  m_cf.sendPwmSetpoint(pwm1, pwm2, pwm3, pwm4);
	  // m_sentSetpoint = true;
  }
...
public:
  ...
  void runFast()
  {
	...
	ROS_INFO("totalLatency: %lf", totalLatency);
